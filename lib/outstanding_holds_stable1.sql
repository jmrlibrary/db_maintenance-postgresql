...
CONCAT ('b' , bib_record_property.bib_record_id, 'a') as BIB_NUM,
concat (patron_view.record_type_code, patron_view.record_num, 'a') as PATRON_NUM,
CONCAT ( item_view.record_type_code, item_view.record_num, 'a') as ITEM_NUM,
max(checkout.due_gmt),
max(checkout.overdue_count),
max(checkout.overdue_gmt),
max(left(item_view.location_code,1) ) AS CHARloc,
min(hold.placed_gmt),
